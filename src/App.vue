      ...
      <div class="head-top">
        <div class="section">
          ...
          <div id="menu" class="right-box">
            ...
            <span>
              <router-link to="/vipcenter" v-show="isShow" class="">会员中心</router-link>
              <strong>|</strong>
              <a @click="logout">退出</a>
              <strong>|</strong>
            </span>
            <router-link to="/shopcart" class="">
              <i ref="shoppingCartCountref" id="shoppingCartCount" class="iconfont icon-cart"></i>购物车(
              <span>
                <!-- 从仓库中取出暴露出来的getBuyCount下面的库存数据 -->
        ...
        <div class="section">
          <div id="menu2" class="nav-box menuhd">
            <ul>
              ...
              <li class="video">
                <a href="#" class="">
                  <span class="out" style="top: 0px;">购物超市</span>
                </a>
              </li>
              <li class="down">
                <a href="#" class="">
...
<style>
/*
*导入jquery里面的css 这里需要末尾用分号; 插件里面的样式要写在前面
*/
@import './statics/jquery/css/style.css';
    ...
    logout(){
      this.$axios.get(`site/account/logout`).then(response=>{
        if(response.data.status==0){
             this.$confirm('是否确认退出到登录页面?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
         
          // 跳到首页 或者跳到登录页面 这里就跳到登录页面
